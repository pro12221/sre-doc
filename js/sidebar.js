...
(function() {
  ...
  function getCurrentCategory() {
    ...
    if (!window.SREData || !window.SREData.findByUrl) return null;
    // data.js stores article URLs as `/posts/{slug}.html` (root-relative). The
    // browser's pathname may or may not have the .html suffix depending on whether
    // Cloudflare's clean-URL rewrite kicked in, so try the bare slug too.
    var base = file.replace(/\.html$/, '');
    var article = window.SREData.findByUrl('/posts/' + base + '.html')
               || window.SREData.findByUrl('/posts/' + base);
  ...
  function render() {
    ...
    for (var i = 0; i < data.categories.length; i++) {
      ...
      for (var j = 0; j < articles.length; j++) {
        ...
        // a.url is root-relative (`/posts/{slug}.html`) so it works from any page depth.
